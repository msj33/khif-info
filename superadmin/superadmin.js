...
let token=null,commandSha=null,statusTimer=null;
const els={loginPanel:$('loginPanel'),superPanel:$('superPanel'),logoutButton:$('logoutButton'),username:$('username'),password:$('password'),loginButton:$('loginButton'),loginMessage:$('loginMessage'),onlinePill:$('onlinePill'),deviceTitle:$('deviceTitle'),lastSeen:$('lastSeen'),hostname:$('hostname'),uptime:$('uptime'),temperature:$('temperature'),browser:$('browser'),lastCommand:$('lastCommand'),lastError:$('lastError'),rawStatus:$('rawStatus'),reloadPageButton:$('reloadPageButton'),restartBrowserButton:$('restartBrowserButton'),rebootPiButton:$('rebootPiButton'),commandMessage:$('commandMessage')};
...
async function login(){try{msg(els.loginMessage,'Logger ind…');els.loginButton.disabled=true;if((els.username.value||'').trim()!==CONFIG.adminUsername)throw new Error('Forkert brugernavn eller password.');token=await decryptToken(els.password.value);els.loginPanel.classList.add('hidden');els.superPanel.classList.remove('hidden');els.logoutButton.classList.remove('hidden');els.password.value='';msg(els.loginMessage,'');await refreshStatus();statusTimer=setInterval(refreshStatus,POLL_MS)}catch(error){token=null;msg(els.loginMessage,friendlyError(error),true)}finally{els.loginButton.disabled=false}}
function init(){msg(els.loginMessage,'Klar til login.');els.loginButton.onclick=login;els.password?.addEventListener('keydown',e=>{if(e.key==='Enter')login()});els.logoutButton.onclick=()=>location.reload();els.reloadPageButton.onclick=()=>sendCommand('reload-page');els.restartBrowserButton.onclick=()=>sendCommand('restart-browser');els.rebootPiButton.onclick=()=>sendCommand('reboot-pi')}
