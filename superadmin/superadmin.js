...
async function login(){try{msg(els.loginMessage,'Logger ind…');els.loginButton.disabled=true;const enteredUsername=(els.username.value||'').trim();const allowedUsernames=[String(CONFIG.adminUsername||'').trim(),...Object.values(CONFIG.superadminUsers||{}).map(v=>String(v||'').trim())].filter(Boolean);if(!allowedUsernames.includes(enteredUsername))throw new Error('Forkert brugernavn eller password.');token=await decryptToken(els.password.value);els.loginPanel.classList.add('hidden');els.superPanel.classList.remove('hidden');els.logoutButton.classList.remove('hidden');els.password.value='';msg(els.loginMessage,'');await Promise.all([refreshStatus(),refreshSchedule()]);statusTimer=setInterval(refreshStatus,POLL_MS)}catch(error){token=null;msg(els.loginMessage,friendlyError(error),true)}finally{els.loginButton.disabled=false}}
