...
function normalizeRole(value){return value==='superadmin'?'superadmin':'admin'}
function loginRecord(username,role,password){return{username:normalizeUsername(username),role:normalizeRole(role),password:String(password||'')}}
function defaultLogins(){return{version:1,users:[loginRecord(CONFIG.adminUsername,'superadmin','')],updatedAt:new Date().toISOString(),updatedBy:'superadmin'}}
function normalizeLogins(logins){const base=defaultLogins();const src=logins&&Array.isArray(logins.users)?logins.users:[];const users=[];src.forEach(entry=>{const username=normalizeUsername(entry?.username);if(!username)return;const role=normalizeRole(entry?.role);const password=String(entry?.password||'');if(!users.some(u=>u.username===username))users.push({username,role,password})});if(!users.some(u=>u.username===normalizeUsername(CONFIG.adminUsername))){users.push(loginRecord(CONFIG.adminUsername,'superadmin',''))}return{version:Number(logins?.version||base.version||1),users,updatedAt:logins?.updatedAt||base.updatedAt,updatedBy:logins?.updatedBy||base.updatedBy}}
function superadminUsernames(){return normalizeLogins(loginsData).users.filter(u=>u.role==='superadmin').map(u=>u.username)}
function adminUsernames(){const users=normalizeLogins(loginsData).users;const admins=users.filter(u=>u.role==='admin').map(u=>u.username);const supers=users.filter(u=>u.role==='superadmin').map(u=>u.username);return Array.from(new Set([...admins,...supers]))}
function renderLoginAdmin(){if(!els.loginList)return;const users=normalizeLogins(loginsData).users;els.loginList.innerHTML=users.map(user=>`<div class="schedule-day"><p class="status-label">${user.username}</p><p class="status-value">${user.role==='superadmin'?'Superadmin (begge paneler)':'Admin (/admin)'}</p><label>Nyt password<input type="password" data-login-password="${user.username}" autocomplete="new-password" /></label><label>Rolle<select data-login-role="${user.username}"><option value="admin" ${user.role==='admin'?'selected':''}>Admin</option><option value="superadmin" ${user.role==='superadmin'?'selected':''}>Superadmin</option></select></label><div class="actions"><button type="button" class="secondary" data-save-login="${user.username}">Gem ændringer</button><button type="button" class="danger" data-remove-login="${user.username}">Fjern login</button></div></div>`).join('');els.loginList.querySelectorAll('[data-save-login]').forEach(btn=>btn.onclick=()=>updateLogin(btn.getAttribute('data-save-login')||''));els.loginList.querySelectorAll('[data-remove-login]').forEach(btn=>btn.onclick=()=>removeLogin(btn.getAttribute('data-remove-login')||''))}
async function refreshLogins(){try{const file=await readJsonFile(LOGINS_PATH);loginsData=normalizeLogins(file.json);loginsSha=file.sha;renderLoginAdmin()}catch(error){loginsData=defaultLogins();renderLoginAdmin();msg(els.loginAdminMessage,`Kunne ikke hente logins: ${friendlyError(error)}`,true)}}
async function saveLogins(){const payload=normalizeLogins(loginsData);payload.updatedAt=new Date().toISOString();payload.updatedBy=currentLoginUsername||'superadmin';loginsSha=await writeJsonFile(LOGINS_PATH,payload,loginsSha);loginsData=payload;renderLoginAdmin()}
async function addLogin(){try{const username=normalizeUsername(els.newLoginUsername?.value);const password=String(els.newLoginPassword?.value||'').trim();const role=normalizeRole(els.newLoginRole?.value);if(!username)throw new Error('Brugernavn mangler.');if(!password)throw new Error('Password mangler.');const data=normalizeLogins(loginsData);if(data.users.some(u=>u.username===username))throw new Error('Brugernavnet findes allerede.');data.users.push(loginRecord(username,role,password));loginsData=data;await saveLogins();els.newLoginUsername.value='';els.newLoginPassword.value='';msg(els.loginAdminMessage,'Login oprettet.')}catch(error){msg(els.loginAdminMessage,friendlyError(error),true)}}
async function updateLogin(username){try{const key=normalizeUsername(username);if(!key)throw new Error('Ugyldigt brugernavn.');const roleInput=els.loginList?.querySelector(`[data-login-role="${key}"]`);const passInput=els.loginList?.querySelector(`[data-login-password="${key}"]`);const newRole=normalizeRole(roleInput?.value);const newPassword=String(passInput?.value||'').trim();const data=normalizeLogins(loginsData);const user=data.users.find(u=>u.username===key);if(!user)throw new Error('Login blev ikke fundet.');user.role=newRole;if(newPassword)user.password=newPassword;if(key===normalizeUsername(CONFIG.adminUsername))user.role='superadmin';loginsData=data;await saveLogins();if(passInput)passInput.value='';msg(els.loginAdminMessage,'Login opdateret.')}catch(error){msg(els.loginAdminMessage,friendlyError(error),true)}}
async function removeLogin(username){try{const key=normalizeUsername(username);if(!key)throw new Error('Ugyldigt brugernavn.');if(key===normalizeUsername(CONFIG.adminUsername))throw new Error('Primær superadmin kan ikke slettes.');if(key===currentLoginUsername)throw new Error('Du kan ikke slette din egen aktive login.');if(!confirm(`Fjern login "${key}"?`))return;const data=normalizeLogins(loginsData);data.users=data.users.filter(u=>u.username!==key);if(!data.users.length)throw new Error('Der skal være mindst ét login.');loginsData=data;await saveLogins();msg(els.loginAdminMessage,'Login fjernet.')}catch(error){msg(els.loginAdminMessage,friendlyError(error),true)}}
...
async function login(){try{msg(els.loginMessage,'Logger ind…');els.loginButton.disabled=true;const enteredUsername=normalizeUsername(els.username.value);const enteredPassword=String(els.password.value||'');token=await decryptToken(enteredPassword);await refreshLogins();const users=normalizeLogins(loginsData).users;const user=users.find(u=>u.username===enteredUsername);if(!user)throw new Error('Forkert brugernavn eller password.');if(!superadminUsernames().includes(enteredUsername))throw new Error('Kun superadmin kan logge ind her.');if(user.password&&user.password!==enteredPassword)throw new Error('Forkert brugernavn eller password.');currentLoginUsername=enteredUsername;els.loginPanel.classList.add('hidden');els.superPanel.classList.remove('hidden');els.logoutButton.classList.remove('hidden');els.password.value='';msg(els.loginMessage,'');await Promise.all([refreshStatus(),refreshSchedule(),refreshLogins()]);statusTimer=setInterval(refreshStatus,POLL_MS)}catch(error){token=null;msg(els.loginMessage,friendlyError(error),true)}finally{els.loginButton.disabled=false}}
...
function init(){msg(els.loginMessage,'Klar til login.');els.loginButton.onclick=login;els.password?.addEventListener('keydown',e=>{if(e.key==='Enter')login()});els.logoutButton.onclick=()=>location.reload();els.reloadPageButton.onclick=()=>sendCommand('reload-page');els.restartBrowserButton.onclick=()=>sendCommand('restart-browser');els.turnScreenOnButton.onclick=()=>sendCommand('screen-on');els.turnScreenOffButton.onclick=()=>sendCommand('screen-off');els.rebootPiButton.onclick=()=>sendCommand('reboot-pi');els.saveScheduleButton.onclick=()=>saveSchedule();els.refreshScheduleButton.onclick=()=>refreshSchedule();els.refreshScheduleOnPiButton.onclick=()=>sendCommand('reload-schedule');els.addLoginButton.onclick=()=>addLogin()}
